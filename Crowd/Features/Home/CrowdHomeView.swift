        ...
        ZStack {
                ...
                .onMapCameraChange { ctx in
                    ...
                    if abs(clamped - ctx.camera.distance) > 1 {
                        cameraPosition = .camera(
                            MapCamera(centerCoordinate: ctx.camera.centerCoordinate,
                                      distance: clamped,
            ...
            GeometryReader { geo in
                VStack(spacing: 0) {
                    ...
                    // Bottom frosted panel + floating buttons + label
                    ZStack {
...
// MARK: - Reusable Bottom Overlay (inline for self-containment)
private struct BottomOverlay<Content: View>: View {
    ...
    var body: some View {
        GeometryReader { geo in
            ...
            .offset(y: isPresented ? 0 : totalH) // slide in/out
    ...
    private func dragGesture(targets: [CGFloat]) -> some Gesture {
        DragGesture(minimumDistance: 2, coordinateSpace: .global)
            .onChanged { value in
                ...
                translation = max(0, dy) // only drag down
            }
            .onEnded { _ in
                defer { translation = 0 }
                let base = targets[clamped: snapIndex]
